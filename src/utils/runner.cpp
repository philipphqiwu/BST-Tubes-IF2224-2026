...
int runMilestone3(const std::string &filename)
{
    ...
    else
    {
        ...
        if (outFile.is_open())
            outFile << "=== SEMANTIC ANALYSIS BERHASIL (TIDAK ADA ERROR) ===\n";
    }

    // Print all three symbol tables (tab, btab, atab)
    printAllSymbolTables(semantic.symtab, std::cout);
    if (outFile.is_open())
        printAllSymbolTables(semantic.symtab, outFile);
...
int runMilestone4(const std::string &filename)
{
    ...
    Interpreter interpreter(icgen.code, semantic.symtab, icgen.getSubprogramStartLine());
    std::ostringstream runtimeOut;
    interpreter.execute(runtimeOut);
    const std::string runtimeText = runtimeOut.str();
    std::cout << "\n=== INTERPRETER OUTPUT ===\n";
    if (!runtimeText.empty())
    {
        std::cout << runtimeText;
    }
    if (outFile.is_open())
    {
        outFile << runtimeText;
